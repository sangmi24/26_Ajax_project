...
public class JqAjaxController2 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// post 방식으로 요청이 넘어올 경우에는 항상
		// 인코딩 설정부터 UTF-8로 지정해줬었음!
		// => AJAX 는 기본 인코딩이 UTF-8임
...
//		response.getWriter().print("나이: "+age);
		...
		// 결론 : ajax 는 기본적으로 결과를 오로지 한개만 응답할 수 있음
		// => 단, 외부 라이브러리를 이용하면 응답 데이터를 여러개 전달 할 수 있음
		 ...
		 *  json-simple-1.1.1.jar 다운로드 후 WEB-INF\lib 폴더로 옮겨놓기
		 *  
		...
		// JSON 을 이용해서 여러개의 응답데이터 보내기
		// 1. JSONArray 를 이용한 방법
//		JSONArray jArr = new JSONArray(); // [] 
//		jArr.add("이름 : "+name); //["이름: 홍길동"]
//		jArr.add("나이 : "+age); //["이름: 홍길동", "나이 : age"]
		
		
		// 참고 자바스크립트의 배열 == 자바의 ArrayList (종류상관없음, 사이즈제한없음, 인덱스개념 있음)
		// ArrayList 에 값을 추가할때 필요한 메소드 : list.add(추가할값);
		
		// 응답할 데이터를 전송시 데이터의 컨텐트타입을 제대로 지정해야
		// 문자열 형식이 아닌 진짜 배열형식으로 넘어가게 됨!
		...
		// Map 에 값을 추가할때 필요한 메소드 : hashMap.put("키",밸류);
