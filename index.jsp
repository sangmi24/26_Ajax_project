...
<body>
  ...
  <p>
     Asynchronous JavaScript And XML 의 약자로 <br>
       ...
       우리가 기존에 a 태그로 요청하거나 form태그로 요청하는 방식은 동기식 요청이였음 <br>
    => 응답페이지가 돌아와야 결과를 볼 수 있음 (페이지 화면이 깜빡거림) <br>
       비동기식 요청을 보내기 위해서는 AJAX 라는 기술이 필요함 <br><br>
       
   * 동기식 / 비동기식
   - 동기식 : 요청 처리 후 그에 해당되는 응답 페이지가 돌아와야만 그 다음 작업도 가능 <br>
                      만약 서버에서 호출된 결과까지의 시간이 지연되면 무작정 계속 기다려야 함 (흰페이지로 보여질것)<br>
                      전체 페이지 자체가 리로드(새로고침)됨, 즉, 페이지가 기본적으로 깜빡거림 <br>
   - 비동기식 : 요청 처리 후 현재페이지를 그대로 유지하면서 중간중간마다 추가적인 요청을 보내 줄 수 있음 <br>
                         요청을 한다고 해서 다른페이지로 넘어가지 않음(현재페이지 그대로임) <br>
                         요청을 보내놓고 그에 해당되는 응답이 돌아올때까지 <br>
                         현재페이지에서 다른 작업을 할 수 있음 <br> <br>
   
   * 사용예시 <br>
       네이버 홈페이지의 실시간 검색어 기능, 검색어 자동완성 기능, 아이디 중복 체크 기능 <br><br>
   ...
   * AJAX 의 구현방식 :  JavaScript 방식 / jQuery 방식(코드가 간결하고 사용하기 쉬움) <br><br>
  ...
  <pre>
    ...
    * 주요속성
    - url : 요청할 url (필수입력사항)=> form태그의 action 속성과 같은 역할
    ...
    - success : ajax 통신 성공시 실행할 내용물을 익명함수로 작성
    	...
    	$.ajax({
    		...
    		success : function(result){
    			...
    			*/
    			...
    			//여러개의 데이터가 JSONObject 에 담겨서 응답된 경우
    			//console.log(result);
    			//result == 객체 == for in 문 활용 가능 / 객체명.속성명으로 속성값 접근 가능
